document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- HELPER FUNCTIONS (gcd, shuffle, drawing, etc.) ---
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                if (!currentUser || !data.users[currentUser]) {
                    ...
                    return reject(new Error("No valid user session."));
                }
                if (!currentMode) {
                    window.location.href = 'mode.html';
                    return reject(new Error("No game mode selected."));
                }
                ...
                // Artificial delay to ensure loading screen is visible and assets can "load"
                setTimeout(() => {
                    resolve();
                }, 500);
    ...
    async function main() {
        try {
            ...
            loadingOverlay.style.display = 'none';
            alert(`A critical error occurred while loading the game: ${e.message}`);
            console.error(e);
            window.location.href = 'index.html';
        }
    }
